import { app, ipcMain, BrowserWindow, shell, IpcMainInvokeEvent } from 'electron';
...
function sha256Base64UrlsafeEncode(word: string) {
  ...
  // base64 encoding
  return sha256Digest.toString('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
}
...
export const CredentialStore: CredentialStoreType = {
  ...
  init: () => {
    ...
    CredentialStore.credentials.dataplant = JSON.parse(
      fs.readFileSync(CredentialStore.credential_file_dataplant, 'utf-8')
    );
    ...
    if (fs.existsSync(CredentialStore.credential_file_additional)) {
      CredentialStore.credentials.additional = JSON.parse(
        fs.readFileSync(CredentialStore.credential_file_additional, 'utf-8')
      );
    } else {
  ...
  addCredentials: (key: string, credentials: AuthWithSecret | AuthIdOnly) => {
    if (CredentialStore.credentialsExist(key)) {
      ...
      fs.writeFileSync(
        CredentialStore.credential_file_additional,
        JSON.stringify(CredentialStore.credentials.additional),
        'utf-8'
      );
    }
  },
  ...
  getHosts: () => {
    return {
      dataplant: Object.keys(CredentialStore.credentials.dataplant),
      additional: Object.keys(CredentialStore.credentials.additional),
    };
  },
};
...
export const DataHubService = {
  ...
  getToken: async (e: IpcMainInvokeEvent | null, code: string, host: string) => {
    ...
    let returnvalue = await InternetService.getWebPageAsJson(null, {
      host: host,
      path: path,
      port: 443,
      method: 'POST',
  ...
  init: async () => {
    ...
    authApp.get('/', async (req: Request, res: Response) => {
      ...
      res.send(
        '<h1>Login and Authorization Complete. You can now return to ARCitect.</h1><script>window.close()</script>'
      );
