...
const GitService = {
  ...
  get_leaf_nodes: (nodes, node) => {
    ...
    if (node.children_.length < 1) return nodes.push(node);
  ...
  build_change_tree: status => {
    ...
    const handler = node => {
      ...
      if (n_selected === 0) select(leaf_nodes);
      else unselect(leaf_nodes);
    ...
    status.forEach(([type, path, size]) => {
      ...
      for (let s = 0; s < segments.length; s++) {
        ...
        node.size += size;
        if (s === segments.length - 1) node.type = type;
        parent = node;
  ...
  get_status: async () => {
    ...
    if (!status_raw[0]) return Error('Unable to access git status');
  ...
  parse_status: async () => {
    ...
    const status = response[1]
      ...
      .slice(0, -1);
    const sizes = await window.ipc.invoke(
      'LocalFileSystemService.getFileSizes',
      status.map(x => ArcControlService.props.arc_root + '/' + x[1])
    );
    for (let i in sizes) status[i].push(sizes[i]);
    ...
    GitService._.change_tree_expanded = ['.'];
    {
      const expand_children = node => {
        if (node.children_.length > 5) {
          ...
          for (let child of node.children) expand_children(child);
        }
      };
      GitService._.change_tree[0].children = GitService._.change_tree[0].children_;
      for (let child of GitService._.change_tree[0].children) expand_children(child);
    }
  },
  ...
  update_lfs_files: async () => {
    if (false) {
      ...
      if (!lfs_files[0]) return console.error('unable to fetch LFS file list');
      ...
      // read git attributes
      const gitattributesRaw = await window.ipc.invoke(
  ...
  patch_remote: url => {
    return AppProperties.user && url.includes(AppProperties.user.host) && GitService.get_url_credentials(url) === ''
      ? `https://oauth2:${AppProperties.user.token.access_token}@${AppProperties.user.host}` +
          url.split(AppProperties.user.host)[1]
      : url;
  },
  ...
  get_branches: async (root = ArcControlService.props.arc_root) => {
    ...
    for (let branch of branches_raw) {
      ...
      if (branch[0] === '*') branches.current = branch_name;
    }
    if (!branches.current) {
      //if there is no branch, we try `git branch --show-current` which works even without commits
  ...
  check_remotes: async (root = ArcControlService.props.arc_root, remotes = GitService._.remotes) => {
    const branches = await GitService.get_branches(root);
    ...
    for (let id in remotes) {
      const url = GitService.patch_remote(remotes[id].url);
      if (AppProperties.user && url.includes(AppProperties.user.host)) {
        ...
        remotes[id].dirty = fetch_response[0] && latest_local_hash !== fetch_response[1].split('\t')[0];
      }
    }
  },

  get_remotes: async root => {
    const response = await window.ipc.invoke('GitService.run', {
      args: [`remote`, `-v`],
      cwd: root || ArcControlService.props.arc_root,
    });
    const remotes = {};
    ...
    if (!root) GitService._.remotes = remotes;
