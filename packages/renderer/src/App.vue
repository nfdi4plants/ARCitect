...
onMounted(async () => {
  ...
  const git_lfs_running = await window.ipc.invoke('GitService.run', {
    args: ['lfs', '--version'],
  });
  ...
  // check if lfs installed
  {
    ...
    for (let check of lfs_checks) if (!git_config.includes(check)) lfs_installed = false;
    if (!lfs_installed) {
    ...
    ([arcRoot, user], [oldArcRoot]) => {
      if (arcRoot && arcRoot !== oldArcRoot && !user) {
        $q.notify({
          ...
          timeout: 5000,
          actions: [{ label: 'X', color: 'black', handler: () => {} }],
        });
      }
    ...
    (newVal, oldVal) => {
      if (!oldVal && newVal) {
        dirtyRemoteNotification = $q.notify({
          type: 'warning',
          message:
            'Your local ARC is out of sync with a remote. You can pull or push changes in the DataHUB Sync view.',
          color: 'red-7',
          textColor: 'white',
          position: 'bottom-left',
          timeout: 0,
...
const restoreGitDialog = () => {
  ...
  const stopWatcher = watch(
    () => AppProperties.git_dialog_state.state,
    newState => {
...
<template>
  <q-layout view="hHh LpR fFf" class="no-selection">
      ...
      <q-list class="column sidebar-container" style="height: 100%; flex-wrap: nowrap">
        ...
        >
          <q-item-section avatar style="height: 2.5em">
            <q-inner-loading :showing="ArcControlService.props.busy" style="width: 4em; background-color: transparent">
              <q-spinner color="white" size="2.5em" :thickness="10" />
            </q-inner-loading>
            <q-inner-loading :showing="!ArcControlService.props.busy" style="width: 4em; background-color: transparent">
              <q-icon size="2.5rem" style="margin: 0 -0.2em" :name="'img:' + logoURL" @click="showHomeView"> </q-icon>
            </q-inner-loading>
          </q-item-section>
          ...
          <a_tooltip> Return to home view </a_tooltip>
        </q-item>
        ...
        <ToolbarButton text="New ARC" icon="note_add" @clicked="newLocalArc()">
          <a_tooltip> Create a new ARC on your computer </a_tooltip>
        </ToolbarButton>
        <ToolbarButton text="Open ARC" icon="file_open" @clicked="openLocalArc()">
          <a_tooltip> Open an existing ARC from your computer </a_tooltip>
        </ToolbarButton>
        <ToolbarButton
          text="Download ARC"
          icon="cloud_download"
          @clicked="AppProperties.state = AppProperties.STATES.OPEN_DATAHUB"
        >
        ...
        <ToolbarButton
          ...
          requiresUser
          requiresGit
          @clicked="AppProperties.state = AppProperties.STATES.GIT_COMMIT"
        >
        ...
        <ToolbarButton
          ...
          requiresUser
          requiresGit
          @clicked="AppProperties.state = AppProperties.STATES.GIT_SYNC"
        >
          <q-icon
            color="red-9"
            name="error"
        ...
        <ToolbarButton
          ...
          requiresARC
          requiresGit
          @clicked="AppProperties.state = AppProperties.STATES.GIT_HISTORY"
        >
        ...
        <ToolbarButton
          ...
          icon="verified"
          requiresARC
          @clicked="AppProperties.state = AppProperties.STATES.VALIDATION"
        >
        ...
        <ToolbarButton
          text="Bugs & Support"
          icon="support_agent"
          @clicked="AppProperties.state = AppProperties.STATES.BUG_REPORT"
        >
          <a_tooltip>Report a bug or ask for help.</a_tooltip>
        </ToolbarButton>
        <ToolbarButton
          text="Services"
          :icon="Object.values(AppProperties.datahub_hosts_msgs).some(x => x.critical) ? 'warning' : 'dns'"
          @clicked="AppProperties.state = AppProperties.STATES.STATUS"
        >
        ...
        <ToolbarButton
          :text="AppProperties.config.toolbarMinimized ? '' : 'Toggle Sidebar'"
          :icon="AppProperties.config.toolbarMinimized ? 'chevron_right' : 'chevron_left'"
          @clicked="AppProperties.config.toolbarMinimized = !AppProperties.config.toolbarMinimized"
        ></ToolbarButton>
        <q-separator />

        <q-item v-ripple clickable dense @click="downloadArcitect">
          <q-item-section avatar>
            <q-icon color="red-9" name="error" v-if="iProps.new_version"> </q-icon>
          </q-item-section>
          <q-item-section style="margin-left: -1.2em">{{ iProps.version }}</q-item-section>
          <a_tooltip v-if="iProps.new_version"> New Version Available! </a_tooltip>
        </q-item>
      </q-list>
    </q-drawer>
    ...
    >
      <q-btn
        style="position: absolute; right: 0em; top: 0.6em; z-index: 9999"
        icon="chevron_right"
        flat
        round
        @click="AppProperties.config.showHelp = false"
      />
    ...
    <q-page-container>
      <q-page padding>
        <q-splitter v-model="iProps.splitterModel" class="full" unit="px" :limits="[300, Infinity]">
          ...
          <template v-slot:after>
                    ...
                    <tbody>
                      <tr>
                        <td style="padding-right: 1em">
                          <q-icon name="warning" size="2em" color="grey-7" />
                        </td>
            ...
            <SwateView
              v-if="AppProperties.load_swate"
              v-show="AppProperties.state === AppProperties.STATES.EDIT_SWATE"
            ></SwateView>
          </template>
        </q-splitter>
      </q-page>
  ...
  <div class="ModalLoading" v-if="ArcControlService.props.super_busy">
    <div>
      <q-circular-progress indeterminate size="20em" color="primary" class="q-ma-md" :thickness="0.6" />
    </div>
  </div>
  ...
  <q-btn
    ...
    fab
    :color="
      AppProperties.git_dialog_state.state === 0
        ? 'secondary'
        : AppProperties.git_dialog_state.state === 1
          ? 'positive'
          : 'negative'
    "
    :icon="
  ...
  >
    <q-tooltip>
      {{
        AppProperties.git_dialog_state.state === 0
          ? 'Git operation running - Click to view'
          : AppProperties.git_dialog_state.state === 1
            ? 'Git operation completed - Click to view'
