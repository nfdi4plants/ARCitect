...
import { toList, collect, removeAt, tryFindIndex, map } from "../../../fable_modules/fable-library.4.1.4/Seq.js";
...
export class ArcInvestigation {
    ...
    TryFindAssay(assayIdentifier) {
        const this$ = this;
        return tryFind((a) => (a.Identifier === assayIdentifier), Array.from(collect((s) => s.Assays, this$.Studies)));
    }
    FindAssay(assayIdentifier) {
        const this$ = this;
        const matchValue = this$.TryFindAssay(assayIdentifier);
        if (matchValue == null) {
            throw new Error(`Unable to find assay with identifier '${assayIdentifier}'.`);
        }
        else {
            return matchValue;
        }
    }
    static tryFindAssay(assayIdentifier) {
        return (inv) => {
            const newInv = inv.Copy();
            return newInv.TryFindAssay(assayIdentifier);
        };
    }
    static findAssay(assayIdentifier) {
        return (inv) => {
            const newInv = inv.Copy();
            return newInv.FindAssay(assayIdentifier);
        };
    }
    Copy() {
        const this$ = this;
